...
const RegisterPage = () => {
  ...
  const [errors, setErrors] = useState({});
  const [otpSent, setOtpSent] = useState(false);  // New state for OTP step
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/register",  // Registration API endpoint
        formData,
        {
          withCredentials: true,  // Include credentials for CSRF and sessions
          headers: {
      ...
      if (response.status === 201) {
        ...
        setOtpSent(true);  // Show OTP input step
      } else {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/verify-otp",  // OTP verification endpoint
